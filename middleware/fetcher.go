...
func Fetch(ctx context.Context, servers map[string]backends.Backend, query string, cache *lru.Cache) ([]byte, error) {
	...
	if len(split) < 3 {
		if len(split) == 2 {
			return fallBackFetch(ctx, servers, split[0], split[1], cache)
		}
	...
	if err != nil {
		return fallBackFetch(ctx, servers, cmd, strings.Join(split[1:], "/"), cache)
	}
	for uid, backend := range servers {
		if buid == uid {
			return backend.Fetch(ctx, cmd, strings.Join(split[2:], "/"))
		}
	}
...
func fallBackFetch(ctx context.Context, servers map[string]backends.Backend, cmd, query string, cache *lru.Cache) ([]byte, error) {
	root := strings.Split(query, "/")[0]
	if cache != nil {
		b, ok := cache.Get(root)
		if ok {
			res, err := servers[b.(string)].Fetch(ctx, cmd, query)
			if err == nil {
				return res, nil
			}
		}
	}
	return parallelFetch(ctx, servers, cmd, query, cache)
}

func parallelFetch(ctx context.Context, servers map[string]backends.Backend, cmd, query string, cache *lru.Cache) ([]byte, error) {
	type Result struct {
		mu  sync.Mutex
		ok  bool
		res []byte
	...
	for _, backend := range servers {
		...
		g.Go(func() error {
			ret, err := backend.Fetch(ctx, cmd, query)
			if err != nil {
				return nil
			}
			res.mu.Lock()
